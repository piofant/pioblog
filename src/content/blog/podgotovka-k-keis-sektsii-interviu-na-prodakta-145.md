---
title: 'Подготовка к кейс-секции интервью на продакта'
pubDate: '2024-04-12'
tgMessageId: 145
heroImage: '/img/og/gradients/podgotovka-k-keis-sektsii-interviu-na-prodakta-145.png'
tags: ['продакт', 'карьера', 'рекомендации']
series: 'yandex-internship'
seriesPart: 3
---

Продуктовая секция чаще всего проходит в формате решения кейсов, и здесь важно сформировать насмотренность, попрактиковаться и отработать механику мышления.

Я изучал продуктовые подходы по статьям и лекциям (во многом [ШМЯ](https://t.me/not_tldr/375)), а затем пробовал применять их в своих проектах на практике. Для развития насмотренности и получения опыта решения кейсов можно разбирать кейсы компаний, брать их можно отсюда:

• [productmanagementexercises.com](https://www.productmanagementexercises.com/interview-questions) — тут вопросы прям с собеседований, и есть ответы пользователей, — это те самые короткие вопросы, где не хватает вводных и нужно снижать уровень неопределенности, очерчивая границу самостоятельно
• [Карьерный цех](https://app.careerpath.tech/contest_inside/1661774275311x182089118701060100?lang=ru_ru) — тут есть большие кейсы компаний и решения участников
- UX/UI кейсы: <https://growth.design/case-studies>
- A/B тесты с результатами: <https://abtest.design>

Также полезно посмотреть тестовые интервью, их можно найти в YT по запросу [product manager mock interview](https://www.youtube.com/results?search_query=product+manager+mock+interview). Например, в [этом](https://www.youtube.com/watch?v=CXdaPETE7iE) видео про кейс с Google maps чувак классно и быстро снижает уровень неопределенности, сужая сегмент пользователей и выбирая конкретную проблему.

А ещё, вот [доклад](https://www.youtube.com/watch?v=MuoR-KJjyHQ) Глеба Кудрявцева про опыт найма продакта и допом [описание](https://t.me/not_tldr/413) текущего статуса рынка продактов от него же.

В след. [посте](/blog/kak-ia-sobral-svoi-keis-klub-dlia-podgotovki-k-interviu-147/) рассказываю про свой кейс-клуб для подготовки к интервью на продакта и как в него вступить

...
const TAG_RULES = [
	...
	{ tag: 'школа жизни', patterns: [ws('школ[аеыу]'), ws('класс'), ws('физтех'), ws('физмат'), ws('универ'), ws('ВУЗ'), ws('ИТМО'), wb('МФТИ'), wb('МИСиС'), wb('МИФИ'), wb('ЦУ'), ws('студен'), ws('абитуриент'), ws('олимпиад')] },
	{ tag: 'вуз',         patterns: [ws('ВУЗ'), ws('бакалавр'), ws('университет'), ws('студент'), /поступлени.{0,15}в.{0,5}топ/iu, ws('ИТМО'), wb('МФТИ'), wb('МИСиС'), wb('МИФИ'), wb('ЦУ'), ws('диплом'), ws('диссертац'), ws('курсовая')] },
	{ tag: 'менторство',  patterns: [ws('менторств'), wb('ментор[ауеы]?'), ws('менти')] },

	// — Сообщество / нетворк —
	{ tag: 'нетворкинг',  patterns: [ws('нетворк'), ws('тус[аоиыeу]'), ws('знакомств'), wb('SNS'), ws('колливинг'), ws('перезнаком'), /рандом.?кофе/iu, /random.?coffee/i] },
	{ tag: 'друзья',      patterns: [ws('друз[ьея]'), ws('дружб'), wb('бро')] },

	// — Психология / рефлексия —
	{ tag: 'рефлексия',   patterns: [ws('рефлекс'), /схема.?терап/iu, wb('КПТ'), /уязвимый.?ребен/iu, /внутренн.{0,5}опор/iu] },
	{ tag: 'психология',  patterns: [ws('психолог'), ws('мантр'), ws('принят'), ws('благодарност'), ws('эмпати'), ws('самооценк'), ws('одиночеств'), ws('страх'), ws('терапи'), wb('ДБТ')] },
	...
	// — Путешествия / трипы —
	{ tag: 'трипы',       patterns: [ws('трип'), ws('автостоп'), ws('путешеств'), /поехал в/iu, ws('поездка'), ws('малошуйк'), /белое море/iu, ws('фестивал')] },

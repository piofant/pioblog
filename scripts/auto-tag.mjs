...
const TAG_RULES = [
	...
	/* «уроки жизни» — семантический тег (формативные экспириенсы, что чему-то научили).
	   Регуляркой не ловится надёжно: ставится ТОЛЬКО руками, авто-теги не трогают.
	   Кандидатов смотри через `node scripts/audit-tags.mjs уроки жизни`. */
	...
	// — Общее —
	/* «жизнь» — только идиоматические словосочетания, бо иначе ловит любое
	   упоминание «в жизни/жизненный» (это половина постов). */
	...
	/* «анонс» — пост анонсирует событие/предлагает услугу с CTA. Не ставится на
	   ретро-упоминания митапов/докладов (на которых уже был). */
	{ tag: 'анонс',       patterns: [
		/\[анонс\]/i,
		...
		/расскажу\s+на/iu,                                                  // «расскажу на любую аудиторию»
		/набираю\s+(?:в|на|группу|команду)/iu,                              // recruitment CTA
	] },
];

const MANAGED_TAGS = new Set(TAG_RULES.map((r) => r.tag));
/* Тег `telegram` исторически клеился синком — выкидываем как шумовой.
   Тег `школа жизни` переименован в `уроки жизни` (см. соответствующий блок выше),
   на старое имя ставим drop, чтобы при retag оно вычищалось. */
const DROP_TAGS = new Set(['telegram', 'школа жизни']);

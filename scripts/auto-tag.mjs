...
const TAG_RULES = [
	...
	/* «школа» — литеральная школьная пора (1-11 классы, физмат, олимпиады).
	   Отдельно от «школа жизни» (уроки жизни) и от «вуз» (университет). */
	{ tag: 'школа',       patterns: [
		/(?:9|10|11)[-\s]?(?:го|й|ый|ом|ого|ому)?\s*клас/iu,         // "10 класс"
		/физмат(?:овск[а-я]+|ный|ная|ной|ном)?\s*класс/iu,            // физмат класс
		ws('физтех'),
		ws('физмат'),
		new RegExp('(?<![\\p{L}\\p{N}])школ[аеыу](?!\\s+жизни)(?![\\p{L}\\p{N}])', 'iu'),
